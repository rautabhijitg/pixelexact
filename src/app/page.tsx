    ...
    return (
        <HomeChrome>
            <main id="main-content">
                <section className="pe__hero"><div className="pe__wrap pe__hero-grid"><div><p className="pe__kicker">UX design and frontend development, from one senior team.</p><h1>Pixel-perfect design. Pixel-perfect code.</h1><p className="pe__hero-sub">Pixel Exact designs and builds your product end-to-end, AI-enabled, delivered in weeks not months, with 40+ years of combined team experience behind every decision.</p><div className="pe__hero-cta"><Button href="/contact" className="pe__button" icon={<ArrowUpRight aria-hidden="true" size={18} />}>Book a consultation</Button><Button href="#work" variant="ghost" className="pe__button" icon={<ArrowDown aria-hidden="true" size={18} />}>View case studies</Button></div></div><div className="artifact pe__ruler-frame"><BrowserChrome /><div className="pe__ruler">{["Research, UX, and UI, handled by one team", "Frontend built to match the design, exactly", "AI-enabled at every stage of the work", "Legacy UI modernized without touching your backend"].map((label, index) => <div className="pe__ruler-line" key={label}><span className="pe__ruler-number">0{index + 1}</span><span>{label}</span></div>)}</div></div></div></section>

                <section className="pe__credibility"><div className="pe__wrap pe__credibility-grid">{[["40+ yrs", "Combined team experience across UX, UI, and frontend engineering."], ["AI-enabled", "Every service is accelerated with AI."], ["Weeks, not months", "Compressed timelines without cutting corners."]].map(([stat, text], index) => <Reveal className="pe__credibility-item" key={stat} index={index}><strong>{stat}</strong><span>{text}</span></Reveal>)}</div></section>
